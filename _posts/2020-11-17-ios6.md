...
//func으로 하면 훨씬 코드가 간결하지만 closure로 만드는 이유는 조건을 closure로 잡아두면 실행하는 순간에 조건을 확인하기가 편하다. 즉, func에서는 조건 변경시 수정이 아주 까다롭다.
...
//괄호를 뒤에 붙이는 이유는 : function뒷부분과 closure가 정의해주는게 똑같다. closure와 함수와 다른점은 이름이 없고type 설정만 해주는 것이다. 함수도 실행을 위해서는 뒤에 ()를 붙여준다. 같은원리로 closure도 실행을 하기위해서는 ()를 붙인다고 보면 된다. function의 구조를 그대로 가져다 사용한다. 원래 마지막도 return { return "h1" + name } 인데 생략해 준 것이다.
```
#### Reference
인프런 강의
